function [v,vprime] = pchiptx(x,y,u)
%PCHIPTX  Textbook piecewise cubic Hermite interpolation.
%  [v,vprime] = pchiptx(x,y,u) finds the shape-preserving piecewise cubic 
%  and first
%  derivative
%  interpolant P(x), with P(x(j)) = y(j), and returns v(k) = P(u(k)).
%  interpolant P'(x), with P'(x(j))=dy(j)/dx(j) and returns 
%  vprime(k)=P'(u(k))
%  See PCHIP, SPLINETX. 
%   Copyright 2012 Cleve Moler and The MathWorks, Inc.
%  First derivatives
   h = diff(x); %computes distance between consecutive nodes
   delta = diff(y)./h;
